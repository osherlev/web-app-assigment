...
const getUserById = async (req: Request, res: Response): Promise<any> => {
    try {
        const user: IUser | null = await User.findById(req.params.id, "-password") as IUser | null;
        if (!user) {
...
const getUserByEmail = async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        const user: IUser | null = await User.findOne({email}, "-password") as IUser | null;
        if (!user) {
            return res.status(404).json({error: "User not found."});
        }
        return res.status(200).json(user);
...
const getUserByUserName = async (req: Request, res: Response): Promise<any> => {
    try {
        const user: IUser | null = await User.findOne({username: req.params.username}, "-password") as IUser | null;
        if (!user) {
            return res.status(404).json({error: "User not found."});
        }
        return res.status(200).json(user);
